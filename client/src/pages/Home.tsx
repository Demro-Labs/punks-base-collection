...
export default function Home() {
  ...
  async function connectMetaMask() {
    const ethereum = (window as any).ethereum;
    if (!ethereum) { toast.error("MetaMask was not detected. Install the extension or use WalletConnect."); return; }
    setConnecting(true);
    try {
  ...
  async function connectWalletConnect() {
    if (!WALLETCONNECT_PROJECT_ID) { toast.info("WalletConnect is ready. Add VITE_WALLETCONNECT_PROJECT_ID in GitHub Actions to enable the QR modal."); return; }
    setConnecting(true);
    try {
      const provider = await EthereumProvider.init({ projectId: WALLETCONNECT_PROJECT_ID, chains: [8453], optionalChains: [8453], showQrModal: true, rpcMap: { 8453: "https://mainnet.base.org" } });
      await provider.connect();
      const accounts = await provider.request({ method: "eth_accounts" }) as string[];
      setWallet(accounts?.[0] || null); setNetwork("Base"); setWcProvider(provider); toast.success("WalletConnect connected on Base.");
    } catch (error: any) { toast.error(error?.message || "WalletConnect connection cancelled."); } finally { setConnecting(false); }
  }

  async function disconnect() { try { await wcProvider?.disconnect(); } catch { /* already disconnected */ } setWallet(null); setWcProvider(null); toast.success("Wallet disconnected."); }
  ...
  return <div className="site-shell">
    ...
    <header className="nav-wrap"><button className="brand" onClick={() => { setView("collection"); scrollTo("top"); }} aria-label="Punks Base Collection"><span className="brand-mark"><img src="/manus-storage/punks-base-mark_650b0dd6.png" alt="" /><i /></span><span>PUNKS<span>/</span>BASE</span></button>
