...
export default function Home() {
  ...
  function imageSrc(item: NftItem) {
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      {view === "collection" && <>
        <section className="hero-section"><div className="hero-copy"><div className="eyebrow"><span className="eyebrow-line" /> ARCHIVE 001 / BASE MAINNET</div><h1>Small format.<br /><em>On-chain</em><br />identity.</h1><p className="hero-lede">Punks Base is a collection of digital characters, recorded on Base and made for people who prefer proof over promises.</p><div className="hero-cta-row"><button className="button button-orange" onClick={() => scrollTo("collection")}>Explore collection <ArrowUpRight size={17} /></button><a className="button button-orange gallery-access-button" href="./gallery">View Full Collection <ArrowUpRight size={17} /></a><button className="button button-ghost" onClick={connectMetaMask}><Wallet size={16} /> Connect MetaMask</button></div><div className="hero-note"><ShieldCheck size={15} /> Public contract · Base network · transparent reading <span className="verified-badge"><img src="./assets/verifier.png" alt="" /> Verified on Base</span></div></div><div className="hero-art-wrap"><div className="art-index">FIG. 001<br /><span>PUNK / BASE</span></div>{nfts[0] ? <img className="hero-art real-nft-image" src={imageSrc(nfts[0])} alt={nfts[0].name} onError={() => markImageFailed(nfts[0].tokenId)} onLoad={() => markImageLoaded(nfts[0].tokenId)} /> : <div className="hero-image-loading">{loadingNfts ? "WAITING FOR REAL TOKEN IMAGE / BASE" : "REAL TOKEN IMAGE UNAVAILABLE"}</div>}<div className="art-caption"><span>01</span><span>ORIGINAL CHARACTER STUDY</span><span>BASE / 8453</span></div></div></section>
