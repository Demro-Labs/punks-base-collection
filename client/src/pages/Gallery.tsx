...
export default function Gallery() {
  ...
  async function loadBatch(start: number, count: number) {
    ...
    const loaded = await readLiveNfts(Array.from({ length: end - start + 1 }, (_, offset) => start + offset), false, (item) => setItems((current) => current.some((entry) => entry.tokenId === item.tokenId) ? current : [...current, item]));
  ...
  async function sync(reset = true) {
    setLoading(true); setError(null); if (!supply) setItems([]); setCursor(1);
    try { const total = await fetchLiveSupply(); setSupply(total); const end = Math.min(total, 12); const loaded = await readLiveNfts(Array.from({ length: end }, (_, index) => index + 1), false, (item) => { setItems((current) => current.some((entry) => entry.tokenId === item.tokenId) ? current : [...current, item]); setLoading(false); }); setItems((current) => current.length ? Array.from(new Map([...current, ...loaded].map((item) => [item.tokenId, item])).values()) : loaded); setCursor(end + 1); setSyncAt(new Date().toISOString()); } catch (cause: any) { setError(cause?.message || "Unable to read the collection from Base."); } finally { setLoading(false); }
  }
